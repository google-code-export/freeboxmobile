        	...
        	try
        	{
				...
				if (getProg)
				{
					...
					if (progress)
					{
						...
						dProgressMessage("Actualisation des programmes TV pour "+max+" chaÃ®nes favorites...",max);
					}
					String channelId;
					for (Iterator <String> it = jChannelsObject.keys() ; it.hasNext() ;)
					{
						...
						for (Iterator <String> jt = jHorairesObject.keys(); jt.hasNext() ;)
						{
							...
							// Je ne sais pas Ã  quoi correspond le channel_id 173 mais les programmes sont vides
				...
				if (getChaines)
				{
					...
					max = jChannelsObject.length();
					dProgressMessage("Actualisation de la liste des "+max+" chaÃ®nes disponibles pour le Guide...",max);
					publishProgress(0);
					for (Iterator <String> it = jChannelsObject.keys() ; it.hasNext() ;)
					{
						publishProgress(courant++);
